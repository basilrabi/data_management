...
class PoliciesAndGuideline(Model):
    """
    Digital log of HR Policies and Guidelines.
    """
    ...
    signed_copy = FileField(upload_to=filepath_policies,null=True, blank=True)
    cancel = BooleanField(null=False, default=False)
    ...
    def save(self, *args, **kwargs):
        if not self.transmittal_number:
            ...
            max_number = PoliciesAndGuideline.objects.filter(transmittal_number__endswith=str(year)).aggregate(Max('transmittal_number'))

            if max_number['transmittal_number__max']:
                max_number = max_number['transmittal_number__max']
            else:
                max_number = f"HRD-POL-0-{year}"
