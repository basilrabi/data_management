...
idling_period <- lapply(1:nrow(equipment_list), function(x) {
  data.frame(
...
lapply(1:nrow(idling_period), function(z) {
  url <- paste0(
    host,
    "history/tracker/list/?trackers=[",
    idling_period$tracker_id[z],
    "]&from=",
    format(idling_period$start[z], format = "%Y-%m-%d%%20%H:%M:%S"),
    "&to=",
    format(idling_period$end[z], format = "%Y-%m-%d%%20%H:%M:%S"),
    "&",
    hash
  )
  api_output <- pull_retry(url)
  cat(sprintf("Processing %s out of %s.\n", z, nrow(idling_period)))
  if (is.data.frame(api_output)) {
    equipment_id <- equipment_list$id[equipment_list$tracker_id == idling_period$tracker_id[z]]
    ...
    if (nrow(output) > 0) {
      ...
      df_idle <- lapply(1:nrow(output), function(i) {
        ...
        dplyr::filter(n < 2)
      table_name <- paste0(idling_period$tracker_id[z],
                           "-",
                           as.Date(idling_period$start[z]),
                           "-idle")
