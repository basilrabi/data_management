...
download_sql () {
    ...
    cmd="\COPY ($sql) TO '$datadir/$1.csv' WITH CSV" && \
...
download_sql comptrollership_activitycategory && \
download_sql comptrollership_activitycode && \
download_sql comptrollership_costcenter && \
download_sql comptrollership_costcenterconversion && \
download_sql comptrollership_costcenterconversion_equipment && \
download_sql comptrollership_generalledgeraccount && \
download_sql comptrollership_material && \
download_sql comptrollership_monthlycost && \
download_sql comptrollership_operationhead && \
...
download_sql custom_unitofmeasure && \
download_sql fleet_additionalequipmentcost && \
download_sql fleet_bodytype && \
download_sql fleet_capacity && \
download_sql fleet_chassisserialnumber && \
download_sql fleet_engineserialnumber && \
...
download_sql fleet_equipmentmobilenumber && \
...
download_sql mine_planning_mapdocumentcontrol && \
download_sql mine_planning_maptype && \
download_sql mine_planning_mineplanningengineer && \
download_sql organization_department && \
download_sql organization_division && \
download_sql organization_manilagpsapikey && \
...
download_sql shipment_draftsurvey && \
